...
with open('Datasets/NYC_crime_5km/NYC_Crime.csv', 'r') as fs:
	fs.readline()
	for line in fs:
...
maxLat = max(latSet)
maxLon = max(lonSet)
latDiv = 111 / 5 # for 5km grid
lonDiv = 84 / 5 # for 5km grid
...
for i in range(len(names)):
	with open('Datasets/NYC_crime_5km/' + names[i], 'wb') as fs:
...
with open('Datasets/NYC_crime_5km/metadata.json', 'w') as fs:
    json.dump(metadata, fs, indent=4)

with open('Datasets/NYC_crime_5km/metadata.pkl', 'wb') as fs:
